...
class TestPubSubClient:
    ...
    def test_on_connect(self, client):
        ...
        with patch.object(client.sio, 'emit') as mock_emit, \
            ...
            mock_emit.assert_called_once_with("subscribe", {
                "consumer": "test_consumer",
                "topics": ["topic1", "topic2"]
            })
    ...
    def test_process_queue_with_handler(self, client):
        ...
        with patch.object(client.sio, 'emit') as mock_emit:
            ...
            mock_emit.assert_called_with("consumed", {
                "consumer": "test_consumer",
                "topic": "topic1",
                "message_id": "msg_123",
                "message": {"data": "test"}
    ...
    def test_start_client(self, client):
        ...
        with patch.object(client.sio, 'connect') as mock_connect, \
                patch.object(client.sio, 'wait') as mock_wait:
            client.start()
